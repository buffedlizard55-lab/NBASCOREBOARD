...
5. **GitHub's cron scheduler is best-effort and has not fired this repo's schedule yet.** Measured: `Sync official NBA data` is active with a `*/10` cron on `main`, and the Actions API reports **0** schedule-triggered runs since the repo was created (every run so far came from a push or a dispatch). The workflow therefore also refreshes on **two cron offsets** and after every Pages deploy (activity-based freshness), and you can always trigger it from **Actions → Sync official NBA data → Run workflow** (`full` for the heavy pass). *Blocker:* cron reliability is GitHub's to provide; the relay path below removes the dependency entirely for live reads.
6. **Playoff/finals dates before 1996** are whatever `nba.com/games?date=…` serves — the archive grows backwards one date at a time, so older nights appear as the cursor reaches them.
7. **This is a read-only mirror of public endpoints.** It is not affiliated with the NBA; respect NBA's terms and rate limits.
