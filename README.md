...
**Historical coverage is incomplete.** The Pages archive contains only dates actually fetched and recorded in [`data/index.json`](data/index.json). On 2026-09-25, after the latest successful branch Action, this checkout contained 116 date files (46 with games), two year-count maps (2025, 2026), 19 game-detail archives and one 2026–27 season schedule. Each full pass attempts up to 30 sequential date pages plus 20 NBA-declared game dates (and seeds missing recent year maps), then rechecks unfinished dates. NBA.com's year-page map supplies game counts for **some** dates (sampled years 1996, 2019, 2024 and 2026); it is *sparse*, omitting many days altogether. An unarchived date is still unknown unless this map has an **explicit** count. A count without a captured date card does not supply scores or PBP. The UI always offers an NBA.com date-page link. The archived [1996 Finals date](https://www.nba.com/games?date=1996-06-16) has official scores but **not** verified CDN player/PBP detail. CDN box/PBP files have only been sampled successfully for some games from the 2019–20 era onward; that is **not** a promise of every game's availability. For example, an audited 2026 Summer League game returned 403 for both CDN detail paths even though [NBA.com shows its official box score](https://www.nba.com/game/gsw-vs-mem-1522600076/box-score); no stats or actions were invented in this archive. Older regular-season PBP is subject to a repository size/retention policy. All-history PBP needs more official coverage and storage than this static repo provides.
...
| Counts for some past dates, **not** scores on uncaptured dates | `allGamesInCurrentYear` on [NBA.com games](https://www.nba.com/games?date=1996-06-16), with omitted keys treated as unknown | [`pages-probe`](data/verification/pages-probe.json) · [`2025 map`](data/calendar/2025.json) · [`2026 map`](data/calendar/2026.json) |
...
```
NBA.com date cards & sparse year counts + NBA CDN scoreboard/box/PBP/schedule
    → scripts/sync_nba_data.py on GitHub Actions (server-side)
    → validated, source-stamped date/calendars/game files in data/
...
2. **Historical completeness and retention.** The new NBA year-date counts guide a source-backed game-day priority queue; their omitted dates remain unknown and still need separate auditing. The daily 30-date sequential + 20-game-day batches do not finish 80 years quickly. Keep track of exactly which game dates and details were checked, and budget storage for full historical PBP. Some pre-2019 NBA detail is absent on the CDN paths tested. Do not promise a full all-time list until the official source and storage are verified.
